class Node:
    ...
    def print_tree(self, order="preorder", level=0):
        if order == "preorder":
            print(" " * (level * 4) + str(self._person))
            for child in self._children:
                child.print_tree(order, level + 1)
        elif order == "inorder":
            if self._children:
                self._children[0].print_tree(order, level + 1)
            print(" " * (level * 4) + str(self._person))
...
def main():
    ...
    while True:
        ...
        if option == "1":
            ...
            print("\nElija el tipo de recorrido:")
            print("1. Preorden")
            print("2. Inorden")
            print("3. Postorden")
            traversal_option = input("Seleccione una opción (1-3): ")
            if traversal_option == "1":
                order = "preorder"
            elif traversal_option == "2":
                order = "inorder"
            elif traversal_option == "3":
                order = "postorder"
            else:
                print("Opción no válida, se usará preorden por defecto.")
                order = "preorder"
            
            print("\nJerarquía completa:")
            root.print_tree(order)
        elif option == "5":
